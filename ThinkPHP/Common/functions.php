...
/**
*传入所有分类数据的数组结构,该函数将会将该结构转换为树形层次分明的数组结构。
*供各个模块内的控制器及逻辑进行使用
...
function ar($arr){
    ...
    foreach($newslist as $value){
        // 先将所有顶级分类先存储到$tree数组里面，作为一维数组结构
        if(0==$value['pid']){
            $tree[]=&$newslist[$value['id']];
        }else if(isset($newslist[$value['id']])){
            $newslist[$value['pid']]['child'][]=&$newslist[$value['id']];
        }
    }
    return $tree;
}
...
function fenye($curpage,$pagenum,$pagess,$urls){
    $fenye='<ul class="pagination">';
    if($curpage>1){
        $fenye .='<li><a href="'.$urls.'/pa/'.(intval($curpage)-1).'">上一页</a></li>';
    }else{
         $fenye .='<li><a href="'.$urls.'/pa/1">上一页</a></li>';
    }
    for($i=1;$i<=$pagess;$i++){
        if($curpage==$i){
            $fenye .='<li class="active"><a href="'.$urls.'/pa/'.$i.'">'.$i.'</a></li>';
        }else{
            $fenye .='<li><a href="'.$urls.'/pa/'.$i.'">'.$i.'</a></li>';
        }
    }
    if(intval($curpage)<intval($pagess)){
        $fenye .='<li><a href="'.$urls.'/pa/'.(intval($curpage)+1).'">下一页</a></li>';
    }else{
        $fenye .='<li><a href="'.$urls.'/pa/'.$pagess.'">下一页</a></li>';
    }
    $fenye .='</ul>';
    return $fenye;
...
*/
    function sendMail($title,$content){
        ...
        $mail->FromName = C('sendrena');
        //smtp登录的账号 这里填入字符串格式的qq号即可
        $mail->Username =C('sendemil');
        //smtp登录的密码 使用生成的授权码（就刚才叫你保存的最新的授权码）
        $mail->Password = C('sendpass');
        //设置发件人邮箱地址 这里填入上述提到的“发件人邮箱”
        $mail->From = C('sendemil');
        //邮件正文是否为html编码 注意此处是一个方法 不再是属性 true或false
        $mail->isHTML(true); 
        //设置收件人邮箱地址 该方法有两个参数 第一个参数为收件人邮箱地址 第二参数为给该地址设置的昵称 不同的邮箱系统会自动进行处理变动 这里第二个参数的意义不大
        $mail->addAddress(C('shouemil'));
        //添加多个收件人 则多次调用方法即可
    ...
    function redafil(){
      $arr=array();
      $file_path="./Application/Home/View";
      $handle=opendir($file_path);
      while(($file=readdir($handle))!==false){
        if($file != '.' && $file != ".." && strpos($file,'Index_') !== false || strpos($file,'List_') !== false){
    ...
    function redaart(){
      ...
      $handle=opendir($file_path);
      while(($file=readdir($handle))!==false){
        if($file != '.' && $file != ".." && strpos($file,'Artic_') !== false){
          array_push($arr,$file);
        }
      }
      closedir($handle);
    ...
    // 将数据处理为树状结构
    function trees($data,$pid=0,$level=0){
      static $arr=array();
      foreach($data as $k=>$v){
        if($v['pid']==$pid){
          $v['level']=$level;
          $arr[]=$v;
          trees($data,$v['id'],$level+1);
        }
      }
      return $arr;
    }
